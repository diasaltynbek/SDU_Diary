...
const Blogs = () => {
  ...
  async function handleDeleteBlog(blog_id) {
    try {
      const response = await axios.delete(
        `https://diplomka-backend.vercel.app/api/teacher/blogs/${blog_id}`
      );
  ...
  async function handleSubmit() {
    ...
    const newPost = {
      ...
      user: storedIds.userId,
      teacher: storedIds.teacherId
    };

    console.log(newPost);

    const response = await axios.post(
      'https://diplomka-backend.vercel.app/api/teacher/blog/add',
      newPost,
      {
        headers: {
          'Content-Type': 'application/json' // Set content type header
                ...
                <div className="card h-100 position-relative">
                  ...
                  <div className="card-body">
                    ...
                    <p className="card-text">
                      Created At: {format(new Date(blog.createdAt), 'yyyy-MM-dd HH:mm')}
                    </p>
                  </div>
                </div>
              </div>
